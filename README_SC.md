...
- 启动时自动显示红色 G6 系统托盘图标。关闭主窗口只会将其隐藏；右键托盘图标可以初始化音频、重新打开主窗口或彻底退出程序。单击左键会在底部面板正上方打开一个行为类似 LXQt 系统音量控件的 G6 专用弹出面板，其中只有扬声器和麦克风两根 0–150% 连续垂直滑杆；悬停或拖动时显示当前百分比，点击面板外部即关闭。双击左键打开主窗口。再次运行 `g6-gui` 不会创建第二个进程或托盘图标，而会唤醒已有窗口并立即返回。
